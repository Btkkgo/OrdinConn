...
真实 Frame Capture、UI Tree、敏感节点脱敏、Observation、类型化 IPC 和关闭链路已经在专用 `OrdinConn_M1_5` AVD 上验证。M2 现已获所有者授权并正在实施；动作领域策略已有单元测试，但导航适配器和真实动作验收尚未验证。在此检查点，生产设备路径仍保持 observe-only。
