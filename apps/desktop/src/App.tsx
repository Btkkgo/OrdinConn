...
import type { AgentMessageDto, AppSnapshotDto, IntelligenceItemDto, MobileWorkspaceDto } from "@ordinconn/contracts";
...
const emptyMobile: MobileWorkspaceDto = { runtimeStatus: "unavailable", adbStatus: "missing", observations: [], feed: [], warehouse: [], strategies: [], settings: { allowedApps: [], screenshotRetention: "memory_only", textScale: 100, researchBudget: { maxDurationSeconds: 300, maxSteps: 40, maxScrolls: 12, maxPages: 20, maxObservations: 50, maxModelCalls: 10 } } };
...
export default function App() {
  ...
  const content = page === "home" ? <MobileHomePage workspace={mobile} signals={snapshot.signals} selectedItemId={selectedItemId} onSelectItem={setSelectedItemId} onObserve={() => void observe()} onOpenDetail={setDetail} t={t}/> : page === "warehouse" ? <WarehousePage workspace={mobile} onOpenDetail={setDetail} t={t}/> : <SettingsPage locale={locale} onLocaleChange={changeLocale} textScale={textScale} onTextScaleChange={changeTextScale} workspace={mobile} providers={snapshot.providers} onSaveProvider={saveProvider} onTestProvider={runtimeClient.testModelProvider} onSaveMobile={async (settings) => { await run(() => runtimeClient.saveMobileSettings(settings)); await refresh(); }} onStrategyChange={async (strategyId, enabled) => { await run(() => runtimeClient.setStrategyEnabled(strategyId, enabled)); await refresh(); }} t={t}/>;
