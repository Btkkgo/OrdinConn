...
## Issue #7 — Merge metadata was part of acceptance

The reviewed M2 tree was unchanged, yet the first public merge failed identity policy. The correct evidence was the commit's Author and Committer classification plus the public reachable-history scan, not the 29 passing behavior checks. A verified signature could not be copied to a new identity: the authorized repair created an unsigned noreply merge with identical tree, parents, message, and timestamps. The old GitHub object remained cached. Future stage close requires pre-merge identity, pre-push history, and post-merge history gates; the account UI alone is insufficient. A worktree `.git` pointer also exposed a scanner boundary bug, fixed with a targeted regression.
